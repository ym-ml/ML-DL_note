...
class Vocab:  #@save
    ...
    def __init__(self, tokens=None, min_freq=0, reserved_tokens=None):
        ...
        self._token_freqs = sorted(counter.items(), key=lambda x: x[1],
                                   reverse=True)  #counter.items():要排序的对象  reverse:降序
        # 未知词元的索引为0
...
def seq_data_iter_random(corpus, batch_size, num_steps):  #@save
    ...
    #正如上图,时间步数为5, 则会有从0,1,2,3,4开始的5中开始情况; 所以这里从随机偏移量开始
    corpus = corpus[random.randint(0, num_steps - 1):]
    
    # 获取子序列数量, 减去1，是因为我们需要考虑标签
...
class RNNModel(nn.Module):
    ...
    def forward(self, inputs, state):
        ...
        ##在一个时间点产生了多个隐状态(Y.shape[-1]个), 将这多个状态通过一个线性模型转换为属于词表中各个词的"概率"
...
def train_epoch_ch8(net, train_iter, loss, updater, device, use_random_iter):
    ...
    for X, Y in train_iter:
        ...
        l = loss(y_hat, y.long()).mean()  # y.long(): turn to long int
        if isinstance(updater, torch.optim.Optimizer):
            updater.zero_grad()
            l.backward()
            grad_clipping(net, 1)
...
#@save
class AdditiveAttention_Ch(nn.Module):
    """加性注意力"""
    ...
    def forward(self, queries, keys, values, valid_lens):
        '''
        返回: (queries.shape[0], queries.shape[1], values.shape[-1])
              bahdanau 中为 (batch_size,1,num_hiddens)
        '''
        queries, keys = self.W_q(queries), self.W_k(keys)
        # 在维度扩展后，
