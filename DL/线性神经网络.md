...
我们需要计算$$\hat y_j = \frac{\exp(o_j)}{\sum_k \exp(o_k)}$$
这面临数值稳定性问题.  
对于上溢,分子分母同除以$\max({\exp(o_k)})$:
$$\begin{split}\begin{aligned}
\hat y_j = \frac{\exp(o_j - \max(o_k))}{\sum_k \exp(o_k - \max(o_k))}.
\end{aligned}\end{split}$$
对于下溢($o_j - \max(o_k)$为极小的负数,导致$\hat y=0$,取对数后为$-inf$),  
我们注意到,在计算softmax之后计算交叉熵损失时,我们需要取对数.所以可以化简:
$$\begin{split}\begin{aligned}
...
我们也希望**保留传统的softmax函数**，以备我们需要评估通过模型输出的概率。但是，我们没有将softmax概率传递到损失函数中，而是在交叉熵损失函数中**传递未规范化的预测**，并同时计算softmax及其对数，这是一种类似“LogSumExp技巧”的聪明方式。
